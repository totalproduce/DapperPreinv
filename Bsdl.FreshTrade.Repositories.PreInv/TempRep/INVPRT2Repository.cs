...
{
    public class InvPrt2Repository : DapperRepositoryBase<DTOInvPrt2>, IInvPrt2Repository
    {
        private const string _sqlQuery = @"SELECT * FROM PREINVTEMP_INVPRT2";

        private const string _sqlDeleteQuery = @"DELETE FROM PREINVTEMP_INVPRT2 WHERE EXTRACTSESSIONID = {0}";

        public InvPrt2Repository(IUnitOfWork unitOfWorkCurrent, ICacheManagerFactory cacheManagerFactory)
            : base
            (
                unitOfWorkCurrent,
                cacheManagerFactory,
        ...
        {
            return INVPRT2.GetPropDefs();
        }

        public override string GetTableName()
        {
            return "PREINVTEMP_INVPRT2";
        }

        public override IBaseModel ToDbModel(DTOInvPrt2 item)
        {
            var result = new INVPRT2();
        ...
        protected override List<DTOInvPrt2> GetDataInternal(List<ISearchField> searchFields, CachingStrategy allowFromCache, uint? topRows)
        {
            ...
            IEnumerable<INVPRT2> results = null;

            try
            {
                results = UnitOfWorkCurrent.Query<INVPRT2>(query);
            }
            catch (Exception ex)
            {
