...
{
    public class InvTotRepository : DapperRepositoryBase<DTOInvTot>, IInvTotRepository
    {
        private const string _sqlQuery = @"SELECT * FROM PREINVTEMP_INVTOT";

        private const string _sqlDeleteQuery = @"DELETE FROM PREINVTEMP_INVTOT WHERE EXTRACTSESSIONID = {0}";

        public InvTotRepository(IUnitOfWork unitOfWorkCurrent, ICacheManagerFactory cacheManagerFactory)
            : base
            (
                unitOfWorkCurrent,
                cacheManagerFactory,
                ...
                {
                   new SearchFieldDef<DTOInvTot, int>(i => i.ExtractSessionID, INVTOT.FieldNames.EXTRACTSESSIONID, FieldType.Integer),
                }
            )
        {
        }
        ...
        {
            return INVTOT.GetPropDefs();
        }

        public override string GetTableName()
        {
            return "PREINVTEMP_INVTOT";
        }

        public override IBaseModel ToDbModel(DTOInvTot item)
        {
            //If you correct anything here - check PreInvTotRepository as well
            DTOInvTotVatInfo 
                vatInfo1 = null, 
                vatInfo2 = null, 
                vatInfo3 = null,
        ...
        protected override List<DTOInvTot> GetDataInternal(List<ISearchField> searchFields, CachingStrategy allowFromCache, uint? topRows)
        {
            ...
            IEnumerable<INVTOT> results = null;

            try
            {
                results = UnitOfWorkCurrent.Query<INVTOT>(query);
            }
            catch (Exception ex)
            {
                throw new DatabaseQueryException("Could not load PREINVTEMP_INVTOT" + Environment.NewLine + ex.Message, ex);
            }
            ...
            if (results != null)
            {
                foreach (var item in results)
                {
                    ...
                    if (item.Cde2.HasValue && (item.Cde2 != 0))
                    {
                        invTot.VatInfo.Add
                            (   item.Cde2.Value,
                                new DTOInvTotVatInfo
                    ...
                    if (item.Cde3.HasValue && (item.Cde3 != 0))
                    {
                        invTot.VatInfo.Add
                            (   item.Cde3.Value,
                                new DTOInvTotVatInfo
        ...
        public List<DTOInvTot> GetByExtractSessionID(int extractSessionID)
        {
            return GetData(
                    new List<ISearchField> {
                        new SearchFieldGeneric<DTOInvTot, int>(i => i.ExtractSessionID).Apply(extractSessionID , FieldType.Integer)
